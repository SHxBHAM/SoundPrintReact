...
console.log(
  "Spotify Client ID:",
  clientId ? `${clientId.substring(0, 4)}...` : "not set"
);
  ...
  try {
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes("403")) {
        throw new Error(
          "Access denied. Please check your Spotify permissions."
        );
      }
...
async function hashGenres(allGenres: string[]): Promise<string> {
  // Sort the array to ensure consistent hashing
  const sortedGenres = [...allGenres].sort();
  const genreString = sortedGenres.join(",");

  // Convert string to Uint8Array
  const encoder = new TextEncoder();
  const data = encoder.encode(genreString);
...
export const getTopGenres = async (limit = 10) => {
  try {
    ...
    // Get the hash
    const genreHash = await hashGenres(allGenres);
    ...
    return { genres: topGenres.join(", "), hash: genreHash }; // ! broder idhr
  } catch (error) {
